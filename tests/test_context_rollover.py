...
import pytest
from context_rollover.models import FrontierPayload, ExecutionStatus
from context_rollover.engine import FrontierDomainEngine
from context_rollover.agents import EventJournalVerifierAgent, StateDeltaCompressorAgent, RolloverTriggerAgent, StateCheckpointerCoordinator
from context_rollover.cli import main


def test_sub_agents():
    a1 = EventJournalVerifierAgent()
...
def test_coordinator():
    coord = StateCheckpointerCoordinator()
    ...
    assert "Deterministic Agent State Machine v1.0" in ans or "specifications" in ans
